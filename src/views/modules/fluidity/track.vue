<template>
  <el-card shadow="never" class="aui-card--fill">
    ...
    >
      ...
      >
        <div v-for="(marker, index) in dayMarkers" :key="index">
          <bm-marker
            :position="{ lng: marker.lng, lat: marker.lat }"
            @click="lookDetail(marker)"
            :icon="{ url: url, size: { width: 100, height: 100 } }"
          >
            <bm-info-window
            ...
            >
              <h5>白天数据</h5>
              <br />
              <h5>出现次数:{{ marker.count }}次</h5>
              <h5>所占比例:{{ marker.proportion * 100 }}%</h5>
            </bm-info-window>
          </bm-marker>
        </div>

        <div v-for="(marker, index) in nightMarkers" :key="index">
          <bm-marker
            :position="{ lng: marker.lng, lat: marker.lat }"
            @click="lookDetail(marker)"
            :icon="{ url: url2, size: { width: 100, height: 100 } }"
          >
            <bm-info-window
              :show="marker.showFlag"
              @close="infoWindowClose(marker)"
...
export default {
  ...
  data() {
    return {
      url: require("../../../assets/img/day.png"),
      url2: require("../../../assets/img/night.png"),
  ...
  methods: {
    ...
    getInfo(row) {
      ...
      this.$http({
        ...
        this.dayMarkers = res.data.day_rule;
        this.nightMarkers = res.data.night_rule;
      });
    },
    //新增
    addok() {
      this.$refs.ruleFormAdd.validate((valid) => {
        ...
        if (
          ...
          this.$http
            .post("/hadoop/task/personTrajectory", data)
            .then(({ data: res }) => {
              if (res.code == 0) {
                this.$message({
                  type: "success",
