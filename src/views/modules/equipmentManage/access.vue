...
<!--			<el-form-item label="学号/工号" prop="userno">
			...
			<el-form-item>
				...
				<el-button type="primary" @click="resetFormHandle()">重置</el-button>
				<el-button  type="primary" @click="addAccess">新建</el-button>
				<el-button  type="primary" @click="sendFaceToEquip">下发指定</el-button>
				<el-button  type="primary" @click="sendFaceAll">下发所有</el-button>
				<el-button  type="primary" @click="sendFaceEmpower">门禁授权</el-button>
				<!--<el-button  type="danger" @click="closeAllDevice">断开所有设备</el-button>
				<el-button   type="danger" @click="updatePassWordHandle">批量修改密码</el-button>-->
			...
			<el-table-column label="操作" align="center" width="330">
				<template slot-scope="scope">
					<div>
						...
						<el-button  v-if="scope.row.aeDevicetype === 1" type="text" @click="optionsHanlder({path:'openRemoteDoor/',id:scope.row.aeId})">远程开门</el-button>
						<el-button  v-if="scope.row.aeDevicetype === 1" type="text" @click="sendHandle(scope.row)">下发所有</el-button>
						<el-button  v-if="scope.row.aeDevicetype === 1" type="text" @click="deleteHandle(scope.row.aeId)" style="color:#f56c6c">删除</el-button>
						<el-dropdown @command="optionsHanlder" v-if="scope.row.aeDevicetype === 1">
							<el-button type="text">
						...
						<!--	<el-dropdown-menu slot="dropdown">
							...
							<el-dropdown-menu slot="dropdown">
								<el-dropdown-item :command="{path:'config',id:scope.row.aeId}">配置信息</el-dropdown-item>
								<el-dropdown-item  :command="{path:'rebootDevice/',id:scope.row.aeId}">重启设备端</el-dropdown-item>
								<el-dropdown-item :command="{path:'openRemoteDoor/',id:scope.row.aeId}">远程开门</el-dropdown-item>
								<el-dropdown-item  :command="{path:'disconnectToDevice/',id:scope.row.aeId}">断开设备</el-dropdown-item>
								<el-dropdown-item   class="danger" :command="{path:'cleanDeviceData/',id:scope.row.aeId}">重置设备</el-dropdown-item>
							</el-dropdown-menu>
						</el-dropdown>
					</div>
				</template>
		...
		<send-face-empower ref="sendFaceEmpower" :aeids="aeIds" accessType="eight"></send-face-empower>
		<!-- 配置信息 -->
...
<script>
	...
	import sendFaceEmpower from './access-send-face-empower'
		...
		methods: {
			...
			sendFaceEmpower(){
				if (this.checkList.length == 0) {
					return this.$message.warning("请选择需要授权的设备！")
				}
				this.$refs['sendFaceEmpower'].init();
			},
			sendFaceToEquip() {
				if (this.checkList.length == 0) {
