...
<!--			<el-form-item label="职工编号" prop="code">
		...
		<el-table :data="dataList" @selection-change="checkListHandle" >
			...
			<el-table-column prop="interviewee" label="通行时间" show-overflow-tooltip align="center">
				<template slot-scope="scope">
					{{scope.row.startTime | formatterDate(scope.row.startTime)}} - {{scope.row.endTime | formatterDate(scope.row.endTime)}}
				</template>
			</el-table-column>
			<el-table-column prop="orderType" label="状态"  show-overflow-tooltip align="center">
				<template slot-scope="scope">
					<el-tag v-if="scope.row.orderType === 1" size="small" type="success">预约</el-tag>
					<el-tag v-else-if="scope.row.orderType === 0" size="small" type="warning">取消预约</el-tag>
				</template>
			</el-table-column>
			<el-table-column prop="reasonUse" label="预约原因" show-overflow-tooltip align="center"></el-table-column>
			<el-table-column prop="operDate" label="申请时间" show-overflow-tooltip align="center"></el-table-column>
		...
		data() {
			...
			return {
				mixinViewModuleOptions: {
					getDataListURL: '/system/park/temp',
