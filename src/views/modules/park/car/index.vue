...
<!--			<el-form-item label="职工编号" prop="code">
		...
		<el-table :data="dataList" @selection-change="checkListHandle" >
			...
			<el-table-column prop="cname" label="姓名" show-overflow-tooltip align="center"></el-table-column>
			<el-table-column prop="carNo" label="车牌号" show-overflow-tooltip align="center"></el-table-column>
			<el-table-column prop="cplx" label="车牌类型" show-overflow-tooltip align="center">
				<template slot-scope="scope">
					<span v-if="scope.row.cplx === 1"> 民用</span>
					<span v-else-if="scope.row.cplx === 2"> 军队</span>
					<span v-else-if="scope.row.cplx === 3"> 警用</span>
					<span v-else-if="scope.row.cplx === 4"> 武警</span>
				</template>
			</el-table-column>
			<el-table-column prop="sflx" label="收费类型" show-overflow-tooltip align="center">
				<template slot-scope="scope">
					<span v-if="scope.row.sflx === 1"> 月卡</span>
					<span v-else-if="scope.row.sflx === 2"> 临时车</span>
					<span v-else-if="scope.row.sflx === 3"> 免费车</span>
					<span v-else-if="scope.row.sflx === 4"> 储值卡</span>
				</template>
			</el-table-column>
			<el-table-column prop="lxdh" label="手机号码" show-overflow-tooltip align="center"></el-table-column>
			<el-table-column prop="ksrq" label="开始日期" show-overflow-tooltip align="center"></el-table-column>
			<el-table-column prop="jzrq" label="截止日期" show-overflow-tooltip align="center"></el-table-column>
			<el-table-column prop="tjcw" label="统计车位" show-overflow-tooltip align="center"></el-table-column>
		...
		data() {
			...
			return {
				mixinViewModuleOptions: {
					getDataListURL: '/system/park/car',
					deleteURL: '/system/park/car',
		...
		methods: {
			...
			cancelHandle(id){
				this.$confirm('是否取消当前申请?', '提示', {
					...
					this.$http({url:'/system/travel/apply/'+id,method:'put'}).then(({ data: res }) => {
						if (res.code === 0) {
							this.$message({
								type: 'success',
								message: '操作成功!'
