<template>
  <el-card shadow="never" class="aui-card--fill">
    ...
    <el-dialog title="详情"  :visible.sync="dialogTableVisible" width="85%">
      <el-tabs v-model="activeName" v-if="dialogTableVisible">
        ...
        <el-tab-pane label="重点热点" name="third">
          <el-table
            :data="
              wifiList.slice(
                (currentPage - 1) * pageSize,
...
export default {
  ...
  methods: {
    ...
    getInfo(item) {
      ...
      this.$http({
        url:
          "/system/keydevicewarning/model?modelId=" +
          item.item +
          "&limit=" +
          this.limit +
          "&page=" +
    ...
    viewHandle(row) {
      ...
      this.$http({
        url:
          "/system/keydevicewarning/modelAll?devKey=" +
          row.devKey +
          "&modelId=" +
          this.modelId,
        method: "get",
