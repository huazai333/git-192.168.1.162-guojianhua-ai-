  ...
  <el-card shadow="never" class="aui-card--fill">
    ...
    >
      ...
      >
        ...
        <el-form-item label="同时空时间范围(秒)" prop="timeRange">
          <el-input
        ...
        <el-form-item label="同时空位置范围(米)" prop="distenceRange">
          <el-input
...
export default {
  ...
  data() {
    return {
      ...
      formAddRules: {
        ...
        timeRange: [
          {
            pattern: /^([1-9][0-9]{0,2}|1800)$/,
            message: "请输入1-1800的数字",
            trigger: "blur",
          },
        ],
        distenceRange: [
  ...
  methods: {
    ...
    addok() {
      this.$refs.ruleFormAdd.validate((valid) => {
        ...
        if (
          ...
          this.$http
            .post("/hadoop/task/devRelationAnalysis", data)
            .then(({ data: res }) => {
              if (res.code == 0) {
                this.$message({
                  type: "success",
    ...
    resetSearch() {
      (this.dataForm = {}),
        (this.data = []),
        this.query(this.dataForm.req_type);
    },
    //取消申请
    cancelHandle(id) {
