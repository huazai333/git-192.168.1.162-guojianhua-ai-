  ...
  <el-card shadow="never" class="aui-card--fill">
    ...
    >
      ...
      <el-form-item label="任务名称" prop="taskName">
        ...
        <el-form-item label="位置范围(米)" prop="distenceRange">
          <el-input
            v-model="addForm.distenceRange"
            placeholder="请输入"
            clearable
          ></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    addok() {
      this.$refs.ruleFormAdd.validate((valid) => {
        ...
        var params = {
            ...
            distenceRange:this.addForm.distenceRange
        };

        let data = JSON.parse(JSON.stringify(params));
        this.$http
          .post("/hadoop/task/positionRelationAnalysis", data)
          .then(({ data: res }) => {
            if (res.code == 0) {
              this.$message({
                type: "success",
    ...
    viewHandle(row) {
      this.$router.push({ name: "position_info", query: { task_id: row.taskId,task_name:row.taskName } });
    },
    handleClose(done) {
