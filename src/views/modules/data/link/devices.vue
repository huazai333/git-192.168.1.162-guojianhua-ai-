<template>
  <el-card shadow="never" class="aui-card--fill">
    <el-form ref="ruleForm" :model="dataForm" label-width="80px" inline>
      <!-- <el-form-item label="接口类型:" prop="req_type">
          <el-select v-model="dataForm.req_type">
           <el-option label="链路分析(设备和设备)" :value="1"></el-option>
            <el-option label="链路分析(设备和热点)" :value="19"></el-option>
            <el-option label="链路分析(热点和热点)" :value="20"></el-option>
          </el-select>
        </el-form-item> -->
    ...
    <el-dialog title="新增" :visible.sync="dialogVisible" width="50%" :before-close="handleClose">
      <el-row>
        <el-col :span="12">
          <el-divider content-position="center">设备1</el-divider>
          <el-form
            ref="ruleFormAdd"
            :model="addForm"
            :rules="formAddRules"
          ...
          >
            ...
            <!-- <el-form-item label="接口类型:" prop="req_type">
          <el-select v-model="addForm.req_type">
            <el-option label="链路分析(设备和设备)" :value="1"></el-option>
            <el-option label="链路分析(设备和热点)" :value="19"></el-option>
            <el-option label="链路分析(热点和热点)" :value="20"></el-option>
          </el-select>
        ...
        <el-col :span="12"
          ><el-divider content-position="center">设备2</el-divider>
          <el-form
            ref="ruleFormAdd"
            :model="addForm2"
            :rules="formAddRules"
...
export default {
  ...
  data() {
    return {
      ...
      loading: false,
      formAddRules: {
        devImsi: [{ required: true, message: "请输入", trigger: "blur" }],
      },
    };
  },
  computed: {
  ...
  methods: {
    ...
    addok() {
      this.$refs.ruleFormAdd.validate((valid) => {
        ...
        var dsd = {
          ...
          device2: {
            devImsi: this.addForm2.devImsi,
            devMac: this.addForm2.devMac,
            devImei: this.addForm2.devImei,
            devPhone: this.addForm2.devPhone,
          },
        };
        var params = {
          req_type: 1,
    ...
    viewHandle(row) {
      // this.$router.push({ name: "device_info", query: row });
    },
    editHandle(row) {
