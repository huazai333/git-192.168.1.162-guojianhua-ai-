...
<!--			<el-form-item label="职工编号" prop="code">
		...
		<el-table :data="dataList" @selection-change="checkListHandle">
			...
			<el-table-column prop="applyUserName" label="申请人账号"  show-overflow-tooltip></el-table-column>
...
<script>
	...
	import viewStaff from './staff-view'
	import {
		getDictLabel
	} from '@/utils'
		...
		data() {
			...
			return {
				mixinViewModuleOptions: {
					getDataListURL: '/system/resident/user/apply',
					deleteURL: '/system/resident/user/apply',
			...
			cancelHandle(id){
				this.$confirm('是否取消当前申请?', '提示', {
					...
					center: true
						...
						this.$http({url:'/system/resident/user/apply/'+id,method:'put'}).then(({ data: res }) => {
							if (res.code === 0) {
								this.$message({
									type: 'success',
									message: '操作成功!'
