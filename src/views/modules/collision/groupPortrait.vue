  ...
  <el-card shadow="never" class="aui-card--fill">
    ...
    >
      ...
      >
        ...
        <el-button type="primary" @click="addWifi" style="margin-bottom: 10px"
          >添加热点</el-button
        >
        <el-button
            type="danger"
            ...
            >删除热点</el-button
          >
        <div v-for="(item, index) in formAddWifi" :key="index" class="jw">
          <el-form-item label="热点Mac" prop="wifiMac">
            <el-input
              v-model="item.wifiMac"
              placeholder="请输入"
              clearable
            ></el-input>
          </el-form-item>
  ...
  methods: {
    ...
    addok() {
      this.$refs.ruleFormAdd.validate((valid) => {
        ...
        this.listWifi=[];
        for (var i = 0; i < this.formAddWifi.length; i++) {
          this.listWifi.push(this.formAddWifi[i]);
        }
        let input = this.list.concat(this.listWifi)
        ...
        this.$http
          .post("/hadoop/task/groupPortraitAnalysis", data)
          .then(({ data: res }) => {
            if (res.code == 0) {
              this.$message({
                type: "success",
    ...
    delInput() {
      ...
      this.formAdd.splice(-1, 1);
    },

    //新增文本框增加按钮
    addWifi(item) {
      this.formAddWifi.push(
        //增加就push进数组一个新值
        {
          wifiMac: "",
        }
    ...
    //新增文本框删除按钮
    delInputWifi() {
      if (this.formAddWifi.length <= 1) {
        return false;
      }
      this.formAddWifi.splice(-1, 1);
    },
  },
};
</script>
