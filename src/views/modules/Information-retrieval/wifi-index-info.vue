<template >
  <div class="detail_page">
    <div class="title">
      <h1>热点：{{ info.ssid }}档案信息<i style="font-size: 14px"> ({{ today }}) </i></h1>
      <div class="qrcode">
...
export default {
  data() {
    return {
      url: require("../../../assets/img/wifi.jpg"),
  ...
  methods: {
    ...
    getDeviceInfo(item) {
        if(item.wifi){
            ...
            this.wifi = item.wifiMac
        }

      this.$http({
        url: "/system/es/wifiFile?wifiMac=" + this.wifi,
        method: "get",
      }).then(({ data: res }) => {
        if (res.code !== 0) {
          return;
        }
        this.info = res.data.wifi;
        this.tagList = res.data.tag;
        this.wifi = res.data.deviceVoList;
        this.devs = res.data.devs.slice(0,9);
      });
    },
     // 跳转设备
    jumpDevice(item) {
