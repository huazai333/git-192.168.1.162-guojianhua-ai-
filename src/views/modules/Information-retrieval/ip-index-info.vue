<template >
  <div class="detail_page">
    <div class="title">
      <h1>
        IP：{{ info.ip }}档案信息<i style="font-size: 14px"> ({{ today }}) </i>
      </h1>
      <div class="qrcode">
        ...
        <div class="list">
          <ul>
            <li
              ...
              :key="index"
              @click="jumpDevice(item)"
            >
              <span> {{ item.count }}次 </span>
              <p>{{ item.brandNew }} {{ item.model }}</p>
              <p>{{ item.phone }}</p>
            </li>
          </ul>
        </div>
...
export default {
  ...
  methods: {
    ...
    getDeviceInfo(item) {
      this.$http({
        url: "/system/es/ipFile?ip=" + item.ip,
        method: "get",
      }).then(({ data: res }) => {
        if (res.code !== 0) {
          return;
